...
// Sample articles data - in production, this would come from the content loading system
...
export default function LeadershipInsightsPage() {
  ...
  return (
    <ContentLayout
      ...
      description="Thoughts on building teams, driving innovation, and leading through complexity"
    >
      <div className="max-w-6xl mx-auto">
        ...
        {filteredArticles.length > 0 ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredArticles.map((article) => (
              <ArticlePreviewCard key={article.slug} article={article} section="leadership-insights" />
            ))}
          </div>
        ) : (
        ...
        {sampleArticles.length === 0 && (
          <div className="text-center py-16">
            <BookOpen className="h-16 w-16 text-slate-500 mx-auto mb-6" />
            <h3 className="text-2xl font-semibold text-slate-300 mb-4">Leadership Insights Coming Soon</h3>
            <p className="text-slate-400 max-w-2xl mx-auto leading-relaxed">
              Check back for thoughts on team building, strategic decision-making, and driving technical excellence. I'm
              currently working on a series of articles covering everything from building high-performance teams to
              navigating complex technical transformations.
            </p>
          </div>
        )}
      </div>
