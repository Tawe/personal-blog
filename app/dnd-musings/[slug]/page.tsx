...
import { Calendar, Clock, ArrowLeft, Share2, Download, ExternalLink } from "lucide-react"
...
export default async function DndContentPage({ params }: PageProps) {
  const decodedSlug = decodeURIComponent(params.slug)
  const content = await getDndContentBySlug(decodedSlug)
  ...
  // Get related content
  const allContent = await getAllDndContent()
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    {relatedContent.map((relatedItem) => (
                      <Link
                        ...
                        href={`/dnd-musings/${relatedItem.slug}`}
